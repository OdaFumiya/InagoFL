...
async function makeTestList(argv) {
    ...
    for (var i = 0; i < files.length; i++) {
        console.log(i + "/" + files.length + ":" + 'node ./node_modules/mocha-list-tests/mocha-list-tests.js ' + files[i]);
        var buf;
        try {
...
async function makeShellscript(testlistJSON) {
    ...
    for (let i = 0; i < Object.keys(testlistJSON).length; i++) {
        console.log(Object.keys(testlistJSON)[i].split('\/').slice(-1)[0])
...
async function parseTestResult(testlistJSON) {
    ...
    for (let i = 0; i < Object.keys(testlistJSON).length; i++) {
        for (let j = 0; j < (testlistJSON[Object.keys(testlistJSON)[i]].tests).length; j++) {
            console.log("./coverage/" + Object.keys(testlistJSON)[i].split('\/').slice(-1)[0] + "_" + j)
            var resultstr = fs.readFileSync("./coverage/" + Object.keys(testlistJSON)[i].split('\/').slice(-1)[0] + "_" + j + "/report.json", 'utf-8')
            var coveragestr = fs.readFileSync("./coverage/" + Object.keys(testlistJSON)[i].split('\/').slice(-1)[0] + "_" + j + "/coverage-final.json", 'utf-8')
